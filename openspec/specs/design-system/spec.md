...
Defines the visual foundation of the portfolio: typography, color palette, design tokens via Style Dictionary, Tailwind CSS v4 integration, and component token uniformity.
...
The system SHALL use Tailwind CSS v4 via `@tailwindcss/vite` with CSS-based configuration. Design tokens SHALL be mapped to Tailwind utilities via the `@theme` directive.
